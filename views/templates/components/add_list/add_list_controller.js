    ...
    $(".add_list_item_button").unbind().click(function(event){
        ...
        if(new_list_item && new_list_item.length > 0){
            ...
            load_template_render_function(template_dir + "/" + template_name, function(status){
                fade_new_content_to_div("#" + display_id, window[template_name + "_tmpl_render_func"]({ file_server_url_prefix: file_server_url_prefix, list_items: existing_list_items, display_id: display_id }))
