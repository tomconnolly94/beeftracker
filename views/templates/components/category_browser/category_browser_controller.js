    ...
    $(".category_button").click(function(event){
        ...
        $.get("/api/events", { match_category: this.id, limit: 6 }, function(data){                
            load_template_render_function(template_dir + "/" + template_name, function(status){
                fade_new_content_to_div("#beef_category_browser_display", window[template_name + "_tmpl_render_func"]({ file_server_url_prefix: "http://res.cloudinary.com/hghz4zts3/image/upload/v1514066941", grid_thumbnail_data: data, data_type: "events" }))
